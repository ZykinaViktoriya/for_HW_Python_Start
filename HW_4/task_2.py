...
a = list(map(int, input('Введите через пробел количество ягод на каждом кусте черники: ').split()))
x = 0
for i in range(1, n - 1):
    if (a[i] + a[i + 1] + a[i - 1] > x):
        x = a[i] + a[i + 1] + a[i - 1]
print(f'Максимальное число ягод, которое можно собрать с трех кустов = {x}')
